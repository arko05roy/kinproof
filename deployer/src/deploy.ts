/**
 * Deploy Kinproof to a Midnight network.
 *
...
import { createKinproofPrivateState, createWitnesses } from '../../contract/src/witnesses.js';
...
const PRIVATE_STATE_ID = 'kinproofPrivateState';
...
const SEED = WALLET.seed;
{
  ...
  if (notice) console.log(`\n  New ${network} deployer wallet created. Recovery material was saved locally with owner-only permissions.\n`);
}
...
const zkConfigPath = path.resolve(__dirname, '..', '..', 'contract', 'managed', 'kinproof');
...
const Kinproof = await import(pathToFileURL(contractPath).href);
...
async function main() {
  console.log('\n╔══════════════════════════════════════════════════════════════╗');
  console.log(`║  Deploy Kinproof to ${network}`);
  console.log('╚══════════════════════════════════════════════════════════════╝\n');
  ...
  for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
    try {
      ...
      deployed = await deployContract(providers, {
        ...
        initialPrivateState: createKinproofPrivateState(randomBytes(32)),
      });
      break;
    } catch (err: any) {
